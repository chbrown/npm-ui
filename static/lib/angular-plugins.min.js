...
replace:!0,scope:{mapObject:"="}}}).directive("onUpload",["$parse",function(d){return{restrict:"A",compile:function(b,a){var c=d(a.onUpload);return function(b,d,h){d.on("change",function(d){b.$apply(function(){var f={$event:d};a.multiple?f.$files=d.target.files:f.$file=d.target.files[0];c(b,f)})})}}}}]).service("$flash",["$rootScope",function(d){return function(b,a){void 0===a&&(a=3E3);d.$broadcast("flash",b,a)}}]).directive("flash",["$timeout","$q",function(d,b){return{restrict:"E",template:'<div class="flash" ng-show="messages.length > 0"><span ng-repeat="message in messages track by $index" ng-bind="message"></span></div>',
